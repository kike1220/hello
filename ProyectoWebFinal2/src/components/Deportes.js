...
export default class Deportes extends Component{
	...
	getNoticias(){
	    ...
	    db.collection("noticias").orderBy("Fecha","desc").get().then((snapShots)=> {
	      snapShots.docs.map( (doc)=>{if(doc.data().Categoria === "Deportes"){
	      	tmpnoticias.push(doc);
	      }})
	      this.setState({deportes: tmpnoticias})
	    ...
	    db.collection("noticias").orderBy("Likes","desc").get().then((snapShots)=> {
	      snapShots.docs.map( (doc)=>{if(doc.data().Categoria === "Deportes"){
	      	tmpndeportes.push(doc);
	      }})
	      this.setState({noticias: tmpndeportes})
	    }).catch(function(error){
	      console.log(error);
	    })
	}
	...
	mostrar1(){
	    var tmp = [];
	    this.state.deportes.map((object,i)=>{
	    	if(i<6){
	    		tmp.push(object);
	    	}
	    })
