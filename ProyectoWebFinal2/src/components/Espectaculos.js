...
import anuncio1 from '../anuncio3.png';
import anuncio2 from '../anuncio4.png';
import anuncio5 from '../anuncio5.png';

export default class Espectaculos extends Component{
	constructor(props){
	    super(props);
	    this.orden = 0;
	    this.state = {
	...
	getNoticias(){
	    ...
	    db.collection("noticias").orderBy("Fecha","desc").get().then((snapShots)=> {
	      snapShots.docs.map( (doc)=>{if(doc.data().Categoria === "Espectaculos"){
	      	tmpnoticias.push(doc);
	      }})
	      this.setState({espectaculos: tmpnoticias})
	    }).catch(function(error){
	      console.log(error);
	    })
	    var tmpnespectaculos = [];
	    db.collection("noticias").orderBy("Likes","desc").get().then((snapShots)=> {
	      snapShots.docs.map( (doc)=>{if(doc.data().Categoria === "Espectaculos"){
	      	tmpnespectaculos.push(doc);
	      }})
	      this.setState({noticias: tmpnespectaculos})
	    }).catch(function(error){
	      console.log(error);
	    })
	}
	...
	mostrar1(){
	    var tmp = [];
	    this.state.espectaculos.map((object,i)=>{
	    	if(i<6){
	    		tmp.push(object);
	    	}
	    })
			...
			<div className="container">
				<div className="row">
				    <div className="col-md-8">
				        <h1 className="my-4">Espectaculos</h1>
